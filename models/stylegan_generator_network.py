...
class MappingModule(nn.Module):
  ...
  def __init__(self,
    ...
    for i in range(num_layers): # IN_DIM OUT_DIM=512
...
class BlurLayer(nn.Module):
  ...
  def forward(self, x):
    ...
    return F.conv2d(x,kernel, stride=1, padding=int((self.kernel.size(2)-1)/2), groups=self.channels)
