...
def training_loop(
    ...
    D=h_layers(config.image_size,fmaps_max=512)
    ...
    # E=nn.SyncBatchNorm.convert_sync_batchnorm(E)
    ...
    for epoch in range(epoch_s,max_epoch):
        ...
        while i<len(train_dataloader):
            ...
            # task_loss_pix = torch.mean((x_s-xrec_s)**2)  # L(x_s,G(E(x_s)))
            ...
            loss_E=loss_pix_weight*task_loss_pix+loss_w_weight*task_loss_w+loss_dst_weight*dst+loss_feat_weight*loss_feat+loss_lpi_weight*loss_lpips
            loss_E.backward()
            ...
            if config.local_rank==0:
                log_message= f"[Task Loss:(pixel){task_loss_pix.cpu().detach().numpy():.3f}, h {task_loss_w.cpu().detach().numpy():.3f},vgg {loss_feat},lpips{loss_lpips.cpu().detach().numpy():.3f}" \
